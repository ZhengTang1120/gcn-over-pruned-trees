...
class BERTtrainer(Trainer):
    ...
    def update(self, batch, epoch):
        ...
        if epoch <= 10:
            logits = self.classifier(h, inputs[1], inputs[8], inputs[9])
            loss += self.criterion(logits, labels)
            for i, f in enumerate(tagged):
                if f:
                    loss += self.criterion2(tagging_output[i], rules[i].unsqueeze(1).to(torch.float32))
                    logits = self.classifier(h[i], inputs[1][i].unsqueeze(0), inputs[8][i], inputs[9][i])
            ...
            for i, f in enumerate(tagged):
                if f:
                    ...
                    # print (n)
                    if n != -1:
                        logits = self.classifier(h[i], tag_cands, torch.cat(n*inputs[8][i]), torch.cat(n*inputs[9][i]))
    ...
    def predict(self, batch, id2label, tokenizer, unsort=True):
        ...
        logits = self.classifier(h, tagging_mask, inputs[8], inputs[9])
        loss = self.criterion(logits, labels)
